...
- Send them the zip directly. 1.6 MB, so WhatsApp and Gmail both take it.
...
|---|---|---|---|
| 1 | Zip is **45 MB**, over Gmail's 25 MB limit. The attachment never arrives | Scan-only package by default: gazetteer only, not the 28 MB ONNX models or 97 MB ort. `--full` restores them | **45 MB → 1.6 MB**, still redacts a PAN on the live tax portal |
...
| 6 | Setup doc still said 45 MB, and did not arm the team for the security question | Rewritten: 1.6 MB, an "expect the security question" section, fake-PAN advice | PDF rebuilt, 4 pages |
...
- `demo/privacy-agent-extension.zip` (45 MB) ← `scripts/package-extension.sh` —
